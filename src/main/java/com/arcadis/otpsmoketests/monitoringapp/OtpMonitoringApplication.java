...
class TestRunner {
  ...
  private void addTestSuite(
    ...
    Gauge
      ...
      .description(
        String.format("Number of tests run in the last run of %s.%s", deploymentName, suiteName)
      )
      .register(meterRegistry);
    Gauge
      .builder(
        String.format("otp.tests.%s.%s.tests_failed", deploymentName, suiteName),
        () -> lastRunSuiteTestsFailed.get(testSuiteKey).get()
      )
      .description(
        String.format("Number of tests failed in the last run of %s.%s", deploymentName, suiteName)
      )
      .register(meterRegistry);
    Gauge
      .builder(
      ...
      .description(String.format("Duration (ms) of the last run of %s.%s", deploymentName, suiteName))
      .register(meterRegistry);
  }
  ...
  public void scheduleTestSuites(Scheduler scheduler) {
    ...
    for (Map.Entry<String, TestSuiteConfig> entry : testSuites.entrySet()) {
      ...
      logger.info("Scheduled test suite {} with interval: {}", testSuiteKey, config.interval());
    }
  }

  private void runSpecificTestSuite(String testSuiteKey, TestSuiteConfig config) {
    ...
    for (Map.Entry<String, TestSuiteConfig> entry : testSuites.entrySet()) {
      ...
      suiteResult
        ...
        .forEach(testResult -> {
          ...
          logger.error(
            "Test failure occurred in suite: " + config.deploymentName() + "." + config.suiteName(),
            StructuredArguments.kv("runId", runId),
